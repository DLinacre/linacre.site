...
const skillGroups = [
  { title: 'Product Engineering', items: ['React', 'TypeScript', 'Vue 3', 'Vite', 'Canvas 2D'] },
  {
    ...
    items: ['Python', 'FastAPI', 'Node.js / Express', 'Docker', 'GitHub Actions'],
  },
  {
    title: 'Mobile & Local-First',
    items: ['Kotlin', 'Android SDK', 'Capacitor', 'Dexie / IndexedDB', 'Offline-first UX'],
  },
  {
...
const principles = [
  [
    ...
    'A project should solve a real, recognizable problem before adding extra frameworks, dashboards, or AI layers.',
  ],
  [
    'Claims Must Be Inspectable',
    'Live links, public source code, clear data provenance, and honest limitations matter more than inflated metrics.',
  ],
  [
    'Local-First Architecture',
    'Browser storage and offline functionality remove server dependence, lower latency, and protect visitor privacy.',
  ],
];
...
export default function About() {
    ...
    <div className="space-y-12">
      ...
      <section className="grid gap-8 overflow-hidden rounded-3xl border border-border-color bg-[var(--linacre-panel)] p-6 shadow-[var(--linacre-card-shadow)] sm:p-8 lg:grid-cols-[.72fr_1.28fr] lg:items-center lg:p-10">
        ...
        >
          ...
          <div className="relative overflow-hidden rounded-3xl border border-border-color bg-background/30 p-3">
            ...
            <div className="mt-3 flex items-center justify-between rounded-xl border border-border-color bg-background/40 px-3 py-2 font-mono text-[10px]">
              <span className="flex items-center gap-1.5 text-emerald-color">
                <span className="h-2 w-2 rounded-full bg-emerald-color animate-pulse" /> Self-Taught
                Software Developer
              </span>
        ...
        >
          <span className="inline-flex items-center gap-2 font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-emerald-color">
            <ShieldCheck className="h-4 w-4" /> About David Linacre
          </span>
          <h1 className="mt-4 font-display text-4xl font-bold tracking-[-0.05em] text-foreground sm:text-5xl">
            16 years of real work before the code.
          </h1>
          <p className="mt-5 max-w-3xl text-base leading-8 text-muted-foreground">
            I am a self-taught software developer based in Barnsley, South Yorkshire. With over 16
            years of hands-on experience in customer service, health & safety compliance, and
            inventory management, I bring a practical, disciplined mindset to software engineering.
          </p>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-muted-foreground">
            I build software designed for real users: browser games like KushCloud, Android debloat
            tools like Linacre Uninstaller, and local-first web applications like Apex POS.
            Everything I ship is open source and open for inspection on GitHub.
          </p>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {principles.map(([title, detail], index) => (
            <motion.article
              ...
              className="rounded-2xl border border-border-color bg-[var(--linacre-panel)] p-5"
            >
              <span className="grid h-10 w-10 place-items-center rounded-xl border border-amber-color/20 bg-amber-color/10 font-mono text-xs font-bold text-amber-color">
                0{index + 1}
              </span>
      ...
      <section className="grid gap-6 lg:grid-cols-[.75fr_1.25fr]">
        <div className="rounded-2xl border border-border-color bg-[#020a11] p-5 font-mono shadow-[var(--linacre-card-shadow)]">
          <div className="flex items-center gap-2 border-b border-white/10 pb-3 text-[10px] text-muted-foreground">
            <span className="h-2.5 w-2.5 rounded-full bg-[#fb7185]" />
            <span className="h-2.5 w-2.5 rounded-full bg-[#38bdf8]" />
            <span className="h-2.5 w-2.5 rounded-full bg-[#34d399]" />
            <span className="ml-2">david_linacre.json</span>
          </div>
  ...
  "motto": "Build, verify, improve"
          ...
          <div className="mt-5 flex items-center gap-2 rounded-xl border border-emerald-color/20 bg-emerald-color/5 p-3 text-[10px] text-emerald-color">
            <MapPin className="h-4 w-4" /> Barnsley, South Yorkshire, United Kingdom
          </div>
        </div>

        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <Code2 className="h-5 w-5 text-amber-color" />
            <h2 className="font-display text-2xl font-bold text-foreground">Skills & Strengths</h2>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {skillGroups.map(group => (
              <article
                ...
                className="rounded-2xl border border-border-color bg-[var(--linacre-panel)] p-5"
              >
                <h3 className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-color">
                  {group.title}
                </h3>
                <ul className="mt-4 space-y-2.5">
                  {group.items.map(item => (
                    ...
                    >
                      <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-amber-color" /> {item}
                    </li>
                  ))}
                </ul>
              </article>
      ...
      <section className="rounded-2xl border border-border-color bg-muted/10 p-6 sm:p-8">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
          <div className="max-w-2xl">
            <span className="inline-flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-emerald-color">
              <Sparkles className="h-4 w-4" /> Open source keeps the work honest
            </span>
            <h2 className="mt-2 font-display text-2xl font-bold text-foreground">
              Try the applications. Inspect the code.
            </h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Every project is built in public with zero hidden tracking or commercial tricks.
            </p>
          </div>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-border-color px-4 py-3 font-mono text-xs font-bold text-foreground hover:border-amber-color/50"
          >
            <Heart className="h-4 w-4 text-emerald-color" /> Support the work
          </a>
        </div>
      </section>
    </div>
