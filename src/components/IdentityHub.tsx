...
const PALETTES: PaletteOption[] = [
  {
    ...
    secondary: '#818CF8',
    description:
      '🌌 Deep obsidian #030712 background with sky cyan & soft indigo border glass glows.',
  },
  {
    id: 'midnight',
...
function MarkTile({
  ...
  return (
    <div className="relative">
      ...
      >
        <span
          className="mx-auto block aspect-square w-full max-w-[72px]"
...
export default function IdentityHub() {
  ...
  const initialTab: MarkTab = isGenFrame(savedFrame)
    ? 'lab'
    : savedFrame.startsWith('custom-')
      ? 'upload'
      : 'curated';

  const [paletteId, setPaletteId] = useState(initialPalette);
  const [customPrimary, setCustomPrimary] = useState(
    () => normalizeHex(safeStorage.get('linacre_brand_custom_primary', '#22D3EE')) || '#22D3EE',
  );
  const [customSecondary, setCustomSecondary] = useState(
    () => normalizeHex(safeStorage.get('linacre_brand_custom_secondary', '#34D399')) || '#34D399',
  );
  const [customBgColor, setCustomBgColor] = useState(() =>
    safeStorage.get('linacre_brand_custom_bg', '#030c14'),
  );
  const [colorTarget, setColorTarget] = useState<ColorTarget>('primary');
  const [recentColors, setRecentColors] = useState<string[]>(() =>
    loadJson<string[]>('linacre_brand_recent_colors', []),
  );
  const [frame, setFrame] = useState(savedFrame);
  const [markTab, setMarkTab] = useState<MarkTab>(
    safeStorage.get('linacre_brand_mark_tab', initialTab) as MarkTab,
  );
  ...
  const [title, setTitle] = useState(
    safeStorage.get('linacre_brand_title', 'Software engineer · useful tools · AI systems'),
  );
  ...
  const [website, setWebsite] = useState(
    safeStorage.get('linacre_brand_website', 'https://www.linacre.site'),
  );
  ...
  const [labFamily, setLabFamily] = useState<GenFamilyId>(
    (safeStorage.get('linacre_lab_family', 'mix') || 'mix') as GenFamilyId,
  );
  const [labSym, setLabSym] = useState(() =>
    Math.min(8, Math.max(2, Number(safeStorage.get('linacre_lab_sym', '4')) || 4)),
  );
  const [labCpx, setLabCpx] = useState(() =>
    Math.min(10, Math.max(1, Number(safeStorage.get('linacre_lab_cpx', '5')) || 5)),
  );
  const [labBatch, setLabBatch] = useState(() =>
    Math.max(0, Number(safeStorage.get('linacre_lab_batch', '0')) || 0),
  );
  const [favMarks, setFavMarks] = useState<string[]>(() =>
    loadJson<string[]>('linacre_brand_fav_marks', []),
  );

  // Custom uploaded emblems & AI prompt icon generation
  const [customEmblems, setCustomEmblems] = useState<CustomEmblem[]>(() =>
    loadJson<CustomEmblem[]>('linacre_custom_emblems', []),
  );
  ...
  const avatarSvg = useMemo(
    () => `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512" style="width:100%;height:100%;display:block">
  ...
  <svg x="96" y="96" width="320" height="320" viewBox="0 0 100 100">${emblem}</svg>
    ...
    [emblem, palette, customBgColor],
  );

  const socialSvg = useMemo(
    () => `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630" style="width:100%;height:100%;display:block">
  ...
  <text x="494" y="450" fill="${palette.primary}" font-family="JetBrains Mono, monospace" font-size="16" font-weight="700">${xmlEscape(website.replace(/^https?:\/\//, ''))}</text>
    ...
    [emblem, name, title, bio, website, palette, customBgColor],
  );
  ...
  const toggleFavourite = (key: string) => {
    setFavMarks(previous =>
      previous.includes(key)
        ? previous.filter(entry => entry !== key)
        : [key, ...previous].slice(0, 24),
    );
  };
  ...
  const readUploads = async (files: FileList | null) => {
    ...
    if (file.size > MAX_UPLOAD_BYTES) {
      setUploadError(
        `“${file.name}” is ${(file.size / 1024).toFixed(0)} KB — keep uploads under 300 KB so they stay fast and storable.`,
      );
      return;
    }
    if (customEmblems.length >= MAX_CUSTOM_EMBLEMS) {
      setUploadError(
        `Up to ${MAX_CUSTOM_EMBLEMS} uploaded marks — delete one before adding another.`,
      );
      return;
    }
    const isSvg = file.type === 'image/svg+xml' || file.name.toLowerCase().endsWith('.svg');
    const isImage =
      isSvg || ['image/png', 'image/jpeg', 'image/webp', 'image/gif'].includes(file.type);
    if (!isImage) {
  ...
  const handleGenerateAiIcon = async (promptToUse?: string) => {
    ...
    try {
      ...
      setAiError(
        err.message ||
          'Could not generate icon. Ensure API key is configured or try another prompt.',
      );
    } finally {
        ...
        <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-muted-foreground">
              Pick any two colours in the spectrum, generate infinite unique emblems in the Emblem
              Lab — free, on-device and deterministic — then preview and export production-ready SVG
              or PNG assets. No accounts, no uploads, no guesswork.
            </p>
          </div>
          <button
        ...
        <div className="space-y-5">
          <section className="rounded-2xl border border-border-color bg-[var(--linacre-panel)] p-5">
                    ...
                    style={
                      active
                        ? {
                            ...
                            boxShadow: `0 0 0 1px ${option.primary}22`,
                          }
                        : undefined
                    }
                  >
                    <span className="flex gap-1">
                      <span
                        className="h-6 w-1/2 rounded-md ring-1 ring-white/10"
                    ...
                    <span className="mt-2 flex items-center justify-between gap-1 font-mono text-[10px] font-bold text-foreground">
                      {option.name}
                      {active && (
                        <Check className="h-3 w-3 shrink-0" style={{ color: option.secondary }} />
                      )}
                    </span>
                  </button>
                );
                ...
                style={
                  palette.id === 'custom'
                    ? {
                        ...
                        borderStyle: 'solid',
                        boxShadow: `0 0 0 1px ${customPrimary}22`,
                      }
                    : undefined
                }
              >
                ...
                <span className="mt-2 flex items-center justify-between gap-1 font-mono text-[10px] font-bold text-foreground">
                  Custom
                  {palette.id === 'custom' && (
                    <Check className="h-3 w-3 shrink-0" style={{ color: customSecondary }} />
                  )}
                </span>
              </button>
            </div>
            ...
            <div className="mt-4 rounded-xl border border-border-color bg-background/25 p-4">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <div
                  className="flex rounded-lg border border-border-color bg-background/40 p-0.5"
                  role="tablist"
                  ...
                  {(['primary', 'secondary'] as ColorTarget[]).map(target => {
                    ...
                    return (
                      <button
                        ...
                        style={
                          active
                            ? { background: `${chip}26`, boxShadow: `inset 0 0 0 1px ${chip}66` }
                            : undefined
                        }
                      >
                        <span
                          className="h-3 w-3 rounded-sm ring-1 ring-white/25"
                    ...
                    <div className="grid grid-cols-5 gap-1.5">
                      {row.colors.map(color => (
                        <SwatchBox
                          key={color}
                          color={color}
                          active={activeTargetColor === color}
                          onPick={pickColour}
                        />
                      ))}
                    </div>
              ...
              {recentColors.length > 0 && (
                <div className="mt-3 flex flex-wrap items-center gap-1.5">
                  <span className="mr-1 font-mono text-[9px] uppercase tracking-wider text-muted-foreground/80">
                    Recent
                  </span>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1 px-2 py-1 rounded-lg border text-[10px] font-mono cursor-pointer transition-all ${
                        customBgColor === bgOption.hex
                          ? 'border-amber-color bg-amber-color/10 text-amber-color font-bold'
                          : 'border-border-color bg-background/30 text-muted-foreground'
                      }`}
                    >
                      <span
                        className="w-2.5 h-2.5 rounded-full border border-white/20"
                        style={{ background: bgOption.hex }}
                      />
                      {bgOption.name}
                    </button>
                  ))}
          ...
          <section className="rounded-2xl border border-border-color bg-[var(--linacre-panel)] p-5">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
              ...
              {isGenFrame(frame) && (
                <span
                  className="inline-flex max-w-full items-center gap-1.5 truncate rounded-lg border border-border-color bg-background/40 px-2 py-1 font-mono text-[9px] font-bold text-muted-foreground"
                  title={frame}
                >
                  <FlaskConical className="h-3 w-3 shrink-0 text-amber-color" />{' '}
                  {labelForKey(frame)}
                </span>
              )}
            </div>
            ...
            <div
              className="mb-4 flex rounded-xl border border-border-color bg-background/30 p-1"
              role="tablist"
                ...
                {FRAMES.map(option => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span className="mt-2 block font-mono text-[10px] font-bold text-foreground">
                        {option.name}
                      </span>
              ...
              <div>
                <div className="mb-3 grid gap-2 sm:grid-cols-[1fr_auto]">
                  <label className="relative block">
                    <span className="mb-1.5 block font-mono text-[9px] font-bold uppercase tracking-wider text-muted-foreground">
                      Seed word — same word, same marks
                    </span>
                    <input
                      value={labWord}
                      onChange={event => {
                        setLabWord(event.target.value.slice(0, 32));
                        setLabBatch(0);
                      }}
                ...
                <div className="mb-3 grid gap-2 sm:grid-cols-3">
                  <label className="block">
                    ...
                    <select
                      value={labFamily}
                      onChange={event => {
                        setLabFamily(event.target.value as GenFamilyId);
                        setLabBatch(0);
                      }}
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      value={labSym}
                      onChange={event => {
                        setLabSym(Number(event.target.value));
                        setLabBatch(0);
                      }}
                      className="mt-2.5 w-full"
                      aria-label="Symmetry folds"
                    />
                  </label>
                  <label className="block">
                    <span className="mb-1.5 block font-mono text-[9px] font-bold uppercase tracking-wider text-muted-foreground">
                    ...
                    <input
                      ...
                      value={labCpx}
                      onChange={event => {
                        setLabCpx(Number(event.target.value));
                        setLabBatch(0);
                      }}
                      className="mt-2.5 w-full"
                ...
                {favMarks.length > 0 && (
                  <div className="mb-3">
                    <div className="mb-1.5 flex items-center justify-between">
                      <span className="font-mono text-[9px] font-bold uppercase tracking-wider text-muted-foreground">
                        Kept marks · {favMarks.length}
                      </span>
                      <button
                        onClick={() => setFavMarks([])}
                        className="font-mono text-[9px] font-bold text-muted-foreground hover:text-error"
                      >
                        Clear all
                      </button>
                    </div>
                ...
                <div className="mt-3 flex items-center justify-between gap-2">
                  <span
                    ...
                    title="Deterministic generator — zero cost, zero uploads"
                  >
                    Endless by design · batch {labBatch + 1}
                  </span>
                  <button
              ...
              <div className="space-y-4">
                <div className="rounded-xl border border-amber-color/30 bg-background/30 p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="inline-flex items-center gap-1.5 font-mono text-[10px] font-bold uppercase tracking-wider text-amber-color">
                      <WandSparkles className="h-3.5 w-3.5 text-amber-color" /> AI Vector Studio ·
                      Gemini 3.6 Flash
                    </span>
                    <span className="font-mono text-[9px] text-emerald-color font-semibold">
                      Live SVG Generator
                    </span>
                  </div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onKeyDown={e => {
                        if (e.key === 'Enter') handleGenerateAiIcon();
                      }}
                ...
                {customEmblems.filter(e => e.id.startsWith('custom-ai-')).length > 0 && (
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {customEmblems
                        .filter(e => e.id.startsWith('custom-ai-'))
                        .map(custom => (
              ...
              <div>
                ...
                >
                  ...
                  <span className="font-mono text-[10px] font-bold text-foreground">
                    Drop or browse — SVG, PNG, JPG, WebP
                  </span>
                  <span className="text-[9px] text-muted-foreground">
                    Under 300 KB · stays on this device · exports with your palette behind it
                  </span>
                </button>
                <input
                  ref={fileInputRef}
                  ...
                  className="hidden"
                  onChange={event => {
                    readUploads(event.target.files);
                    event.target.value = '';
                  }}
                />
                {uploadError && (
                  <p className="mt-2 font-mono text-[10px] text-error">{uploadError}</p>
                )}
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <div>
                <div className="mb-2 flex items-center justify-between font-mono text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                  <label htmlFor="brand-glow">Glow</label>
                  <span style={{ color: palette.primary }}>{glow}/4</span>
                </div>
                <input
                  id="brand-glow"
                  type="range"
          ...
          <section className="rounded-2xl border border-border-color bg-[var(--linacre-panel)] p-5">
            ...
            <div className="mt-4 space-y-3">
              <label className="block">
                <span className="mb-1.5 block font-mono text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                  Display name
                </span>
                <input
                  value={name}
                  onChange={event => setName(event.target.value.toUpperCase())}
                  className="h-11 w-full rounded-xl border border-border-color bg-background/40 px-3 font-mono text-xs text-foreground focus:border-amber-color focus:outline-none"
                />
              </label>
              <label className="block">
                <span className="mb-1.5 block font-mono text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-xl border border-border-color bg-background/40 px-3 text-xs text-foreground focus:border-amber-color focus:outline-none"
                />
              </label>
              <label className="block">
                <span className="mb-1.5 block font-mono text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full resize-none rounded-xl border border-border-color bg-background/40 p-3 text-xs leading-5 text-foreground focus:border-amber-color focus:outline-none"
                />
              </label>
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="block">
                  ...
                  <input
                    ...
                    className="h-11 w-full rounded-xl border border-border-color bg-background/40 px-3 font-mono text-[10px] text-foreground focus:border-amber-color focus:outline-none"
                  />
                </label>
                <label className="block">
                  <span className="mb-1.5 block font-mono text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
        ...
        <div className="space-y-5 lg:sticky lg:top-24">
          <section className="overflow-hidden rounded-2xl border border-border-color bg-[var(--linacre-panel)] shadow-[var(--linacre-card-shadow)]">
            <div className="flex flex-col justify-between gap-3 border-b border-border-color bg-muted/20 p-3 sm:flex-row sm:items-center">
              ...
              <div
                className="flex rounded-xl border border-border-color bg-background/40 p-1"
                role="tablist"
            ...
            <div className="grid gap-2 border-t border-border-color p-4 sm:grid-cols-3">
              <button
                onClick={() => downloadText(currentAsset.svg, `${currentAsset.name}.svg`)}
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-color px-3 py-2.5 font-mono text-[10px] font-bold text-[#031018] hover:bg-amber-glow"
              >
              ...
              <button
                onClick={() =>
                  svgToPng(
                    currentAsset.svg,
                    `${currentAsset.name}.png`,
                    currentAsset.width,
                    currentAsset.height,
                  )
                }
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-border-color bg-muted/20 px-3 py-2.5 font-mono text-[10px] font-bold text-foreground hover:border-amber-color/50"
              >
                <FileImage className="h-4 w-4" /> PNG
              </button>
              <button
                onClick={() => copy(currentAsset.svg, 'asset')}
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-border-color bg-muted/20 px-3 py-2.5 font-mono text-[10px] font-bold text-foreground hover:border-amber-color/50"
              >
                {copied === 'asset' ? (
          ...
          <section className="rounded-2xl border border-border-color bg-[var(--linacre-panel)] p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <h2 className="font-display text-base font-bold text-foreground">
                  Export the full asset set
                </h2>
                <p className="mt-1 text-[11px] text-muted-foreground">
                  Exact dimensions, transparent-safe SVG, and crisp PNG rendering.
                </p>
              </div>
              ...
              {[
                {
                  ...
                  note: 'Scalable mark',
                  icon: Sparkles,
                  action: () => downloadText(emblem, 'linacre-emblem.svg'),
                },
                {
                  label: 'GitHub banner',
                  note: '1280 × 320 PNG',
                  icon: Github,
                  action: () => svgToPng(bannerSvg, 'linacre-github-banner.png', 1280, 320),
                },
                {
                  label: 'Social card',
                  note: '1200 × 630 PNG',
                  icon: ImageIcon,
                  action: () => svgToPng(socialSvg, 'linacre-social-card.png', 1200, 630),
                },
                {
                  label: 'Avatar',
                  note: '512 × 512 PNG',
                  icon: CircleUserRound,
                  action: () => svgToPng(avatarSvg, 'linacre-avatar.png', 512, 512),
                },
              ].map(asset => (
                <button
                  key={asset.label}
                  onClick={asset.action}
                  className="group flex items-center gap-3 rounded-xl border border-border-color bg-background/25 p-3 text-left hover:border-amber-color/45 hover:bg-muted/20"
                >
                  <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-amber-color/10 text-amber-color">
                    <asset.icon className="h-4 w-4" />
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block font-mono text-[10px] font-bold text-foreground">
                      {asset.label}
                    </span>
                    <span className="mt-0.5 block text-[9px] text-muted-foreground">
                      {asset.note}
          ...
          <section className="rounded-2xl border border-border-color bg-[var(--linacre-panel)] p-5">
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Code2 className="h-4 w-4 text-amber-color" />
                <h2 className="font-display text-base font-bold text-foreground">Brand tokens</h2>
              </div>
              <button
                onClick={() => copy(cssTokens, 'tokens')}
                className="inline-flex items-center gap-1.5 font-mono text-[10px] font-bold text-amber-color hover:text-amber-glow"
              >
                {copied === 'tokens' ? (
                ...
                {copied === 'tokens' ? 'Copied' : 'Copy CSS'}
              </button>
            </div>
            <pre className="overflow-x-auto rounded-xl border border-border-color bg-[#020a11] p-4 font-mono text-[10px] leading-5 text-[#9ab7c3]">
              {cssTokens}
            </pre>
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              <button
                onClick={copyThemeLink}
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-border-color px-3 py-2.5 font-mono text-[10px] font-bold text-foreground hover:border-amber-color/50"
              >
                {copied === 'link' ? (
                  <Check className="h-4 w-4 text-emerald-color" />
                ) : (
                ...
                {copied === 'link' ? 'Theme link copied' : 'Copy theme link'}
              </button>
              <button
                onClick={applyCyberPreset}
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-border-color px-3 py-2.5 font-mono text-[10px] font-bold text-foreground hover:border-amber-color/50"
        ...
        {[
          {
            icon: Github,
            title: 'GitHub',
            text: 'Use the wide banner and geometric DL mark. Keep README headers short and useful.',
          },
          {
            icon: Mail,
            title: 'Email and documents',
            text: `Use ${email} and the CyberBlue palette on a plain dark or white surface.`,
          },
          {
            icon: ImageIcon,
            title: 'Social profiles',
            text: 'Use the square avatar at small sizes and the 1200 × 630 card for shared links.',
          },
        ].map(rule => (
