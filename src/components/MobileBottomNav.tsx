...
export default function MobileBottomNav({ activeTab, setActiveTab, openMore }: MobileBottomNavProps) {
  ...
  return (
    <nav className="fixed inset-x-3 bottom-3 z-50 rounded-2xl border border-[var(--linacre-border)] bg-[#0b1018] p-1.5 shadow-[var(--linacre-shadow-float)] backdrop-blur-xl md:hidden" aria-label="Mobile navigation">
      <div className="grid grid-cols-5 gap-1">
        ...
        <button type="button" onClick={openMore} className="flex min-h-12 min-w-11 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-semibold text-[#cbd5e1] transition-colors hover:text-white" aria-label="Open more navigation">
