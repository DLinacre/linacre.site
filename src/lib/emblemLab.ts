...
export type GenFamilyId =
  'prism' | 'sigil' | 'circuit' | 'orbit' | 'woven' | 'wave' | 'glyph' | 'array' | 'mix';
...
export const GEN_FAMILIES: ReadonlyArray<{
  id: Exclude<GenFamilyId, 'mix'>;
  name: string;
  blurb: string;
}> = [
...
export function makeGenKey(config: GenConfig): string {
  const family =
    config.family === 'mix'
      ? 'mix'
      : FAMILY_IDS.includes(config.family as never)
        ? config.family
        : 'sigil';
...
export function parseGenKey(key: string): GenConfig {
  ...
  const family: GenFamilyId =
    familyRaw === 'mix' || FAMILY_IDS.includes(familyRaw as never) ? familyRaw : 'mix';
  return {
    family,
...
export function labelForKey(key: string): string {
  ...
  const name =
    cfg.family === 'mix'
      ? 'Surprise Mix'
      : GEN_FAMILIES.find(f => f.id === cfg.family)?.name || cfg.family;
...
function renderPrism(rng: Rng, cfg: GenConfig, c: Palette): string {
  ...
  if (cpx >= 8)
    parts.push(
      `<circle cx="50" cy="50" r="47" fill="none" stroke="${c.s}" stroke-width="0.9" stroke-dasharray="1 6" opacity="0.5"/>`,
    );
  parts.push(
    `<polygon points="${polygonPoints(sym, 42, offset)}" fill="none" stroke="${c.p}" stroke-width="3" stroke-linejoin="round" ${c.pulse} ${c.glow}/>`,
  );
  parts.push(
    `<polygon points="${polygonPoints(sym, 33.5, offset + 180 / sym)}" fill="none" stroke="${c.s}" stroke-width="1" stroke-dasharray="5 4" opacity="0.4"/>`,
  );

  for (let i = 0; i < sym; i++) {
    const [x, y] = polar(offset + (360 / sym) * i, 42);
    parts.push(
      `<line x1="${n(x)}" y1="${n(y)}" x2="50" y2="50" stroke="${c.s}" stroke-width="0.75" opacity="0.28"/>`,
    );
    if (cpx >= 3) parts.push(`<circle cx="${n(x)}" cy="${n(y)}" r="2" fill="${c.s}"/>`);
  }
  if (cpx >= 5) {
    parts.push(
      `<polygon points="${polygonPoints(sym, 19, offset + 180 / sym)}" fill="none" stroke="${c.p}" stroke-width="1.5" opacity="0.6"/>`,
    );
  }
  parts.push(
    `<polygon points="${polygonPoints(sym, 10, offset)}" fill="url(#brandGrad)" stroke="#ffffff" stroke-opacity="0.35" stroke-width="0.75" ${c.glow}/>`,
  );
  return parts.join('\n  ');
}
...
function renderSigil(rng: Rng, cfg: GenConfig, c: Palette): string {
  ...
  if (cpx >= 6)
    parts.push(
      `<circle cx="50" cy="50" r="44" fill="none" stroke="${c.s}" stroke-width="1" stroke-dasharray="4 5" opacity="0.35"/>`,
    );
  ...
  parts.push(
    `<path d="M 50,${n(40 - cpx)} L 56,50 L 50,${n(60 + cpx)} L 44,50 Z" fill="${c.p}" opacity="0.9" ${c.glow}/>`,
  );
  return parts.join('\n  ');
}
...
function renderCircuit(rng: Rng, cfg: GenConfig, c: Palette): string {
  ...
  if (cpx >= 7)
    parts.push(
      `<rect x="11" y="11" width="78" height="78" rx="11" fill="none" stroke="${c.p}" stroke-width="2" opacity="0.4"/>`,
    );
  ...
  for (let i = 0; i < sym; i++) {
    ...
    vias.push(
      `<circle cx="${n(ex)}" cy="${n(ey)}" r="1.1" fill="${c.s}" opacity="0.9" stroke="none"/>`,
    );
  }
  ...
  if (cpx >= 8) {
    for (let i = 0; i < 4; i++) {
      ...
      parts.push(
        `<rect x="${n(x - 1.6)}" y="${n(y - 1.6)}" width="3.2" height="3.2" fill="none" stroke="${c.s}" stroke-width="0.8" opacity="0.7"/>`,
      );
    }
  }
  parts.push(
    `<circle cx="50" cy="50" r="${n(4 + pulseGroup * 0.6)}" fill="url(#brandGrad)" ${c.pulse} ${c.glow}/>`,
  );
  return parts.join('\n  ');
}
...
function renderOrbit(rng: Rng, cfg: GenConfig, c: Palette): string {
  ...
  parts.push(
    `<circle cx="50" cy="50" r="44" fill="none" stroke="${c.p}" stroke-width="2.4" ${c.pulse} ${c.glow}/>`,
  );
  parts.push(
    `<circle cx="50" cy="50" r="37.5" fill="none" stroke="${c.s}" stroke-width="1" stroke-dasharray="4 4" opacity="0.4"/>`,
  );

  const arcStart = rng.range(0, 360);
  const arcLen = rng.range(50, 120) + cpx * 6;
  parts.push(
    `<path d="${arcPath(44, arcStart, arcLen)}" fill="none" stroke="${c.s}" stroke-width="2.6" stroke-linecap="round"/>`,
  );

  if (cpx >= 7) {
    parts.push(
      `<circle cx="50" cy="50" r="30" fill="none" stroke="${c.p}" stroke-width="1" stroke-dasharray="2 5" opacity="0.5"/>`,
    );
  }
  ...
  if (cpx >= 4) {
    ...
    parts.push(
      `<circle cx="${n(ax)}" cy="${n(ay)}" r="3.8" fill="none" stroke="${c.s}" stroke-width="1.4"/>`,
    );
  }

  parts.push(`<circle cx="50" cy="50" r="9" fill="url(#brandGrad)" ${c.glow}/>`);
  parts.push(
    `<path d="M 50,43.4 L 51.8,48.2 L 56.6,50 L 51.8,51.8 L 50,56.6 L 48.2,51.8 L 43.4,50 L 48.2,48.2 Z" fill="#ffffff" opacity="0.95"/>`,
  );
  return parts.join('\n  ');
}
...
function renderWoven(rng: Rng, cfg: GenConfig, c: Palette): string {
  ...
  if (cpx >= 8)
    parts.push(
      `<circle cx="50" cy="50" r="47" fill="none" stroke="${c.p}" stroke-width="0.9" stroke-dasharray="1 6" opacity="0.5"/>`,
    );
  ...
  if (step > 1) {
    ...
    parts.push(
      `<path d="${d} Z" fill="none" stroke="${c.p}" stroke-width="2.2" stroke-linejoin="round" opacity="0.95" ${c.pulse} ${c.glow}/>`,
    );
  } else if (sym >= 6 && sym % 2 === 0) {
    // Composite star {sym / (sym/2)}: e.g. hexagram = two rotated polygons.
    const half = sym / 2;
    parts.push(
      `<polygon points="${polygonPoints(half, r1, offset)}" fill="none" stroke="${c.p}" stroke-width="2.2" stroke-linejoin="round" ${c.pulse} ${c.glow}/>`,
    );
    parts.push(
      `<polygon points="${polygonPoints(half, r1, offset + 180 / sym)}" fill="none" stroke="${c.p}" stroke-width="2.2" stroke-linejoin="round" opacity="0.75" ${c.glow}/>`,
    );
  } else {
    parts.push(
      `<circle cx="50" cy="50" r="${r1}" fill="none" stroke="${c.p}" stroke-width="2.2" ${c.pulse} ${c.glow}/>`,
    );
  }

  if (cpx >= 2) {
    parts.push(
      `<polygon points="${polygonPoints(sym, 29, offset + 180 / sym)}" fill="none" stroke="${c.s}" stroke-width="1.2" stroke-dasharray="3 4" opacity="0.5"/>`,
    );
  }
  if (cpx >= 4) {
    for (let i = 0; i < sym; i++) {
  ...
  if (cpx >= 6) {
    for (let i = 0; i < sym; i++) {
      ...
      parts.push(
        `<line x1="${n(x1)}" y1="${n(y1)}" x2="${n(x2)}" y2="${n(y2)}" stroke="${c.p}" stroke-width="1" opacity="0.5"/>`,
      );
    }
  }
...
function renderWave(rng: Rng, cfg: GenConfig, c: Palette): string {
  ...
  if (cpx >= 5) {
    parts.push(
      `<path d="M 17,38 V 62 M 83,38 V 62" stroke="${c.s}" stroke-width="2" stroke-linecap="round" opacity="0.6"/>`,
    );
  }
  if (cpx >= 8) {
    parts.push(
      `<path d="M 24,50 H 34 M 66,50 H 76" stroke="${c.p}" stroke-width="2.4" stroke-linecap="round" opacity="0.7"/>`,
    );
  }
...
function renderGlyph(rng: Rng, cfg: GenConfig, c: Palette): string {
  ...
  if (cpx >= 4) {
    parts.push(
      `<path d="M 18,30 V 18 H 30 M 70,18 H 82 V 30 M 82,70 V 82 H 70 M 30,82 H 18 V 70" fill="none" stroke="${c.s}" stroke-width="1.6" stroke-linecap="round" opacity="0.8"/>`,
    );
  }
  if (cpx >= 7) {
    parts.push(
      `<circle cx="50" cy="50" r="45" fill="none" stroke="${c.p}" stroke-width="0.9" stroke-dasharray="2 6" opacity="0.4"/>`,
    );
  }
...
function renderArray(rng: Rng, cfg: GenConfig, c: Palette): string {
  ...
  if (cpx >= 7)
    parts.push(
      `<rect x="13" y="13" width="74" height="74" rx="12" fill="none" stroke="${c.s}" stroke-width="1" stroke-dasharray="4 5" opacity="0.4"/>`,
    );
  ...
  for (let j = 0; j < 4; j++) {
    for (let i = 0; i < 4; i++) {
      ...
      if (i < 3 && active(i + 1, j))
        links.push(
          `<line x1="${n(x + size)}" y1="${n(y + size / 2)}" x2="${n(positionsX[i + 1])}" y2="${n(y + size / 2)}"/>`,
        );
      if (j < 3 && active(i, j + 1))
        links.push(
          `<line x1="${n(x + size / 2)}" y1="${n(y + size)}" x2="${n(x + size / 2)}" y2="${n(positionsY[j + 1])}"/>`,
        );
      ...
      tiles.push(
        `<rect x="${n(x)}" y="${n(y)}" width="${size}" height="${size}" rx="2" fill="${fill}" opacity="${opacity}"/>`,
      );
    }
  }
  ...
  parts.push(
    `<rect x="45.5" y="45.5" width="9" height="9" rx="2" fill="url(#brandGrad)" ${c.glow}/>`,
  );
  return parts.join('\n  ');
}
 ...
 */
export function getGenerativeEmblemContents(
  key: string,
  p: string,
  s: string,
  pulseMotion: boolean,
