...
function isAllowedOrigin(origin: string): boolean {
  ...
  try {
    ...
    if (
      protocol === 'https:' &&
      (hostname.endsWith('-linacres-projects.vercel.app') ||
        hostname === 'linacre-site-repo.vercel.app')
    )
...
function chatInputGuard(req: Request, res: Response, next: NextFunction) {
  ...
  if (
    prompt !== undefined &&
    (typeof prompt !== 'string' || prompt.length === 0 || prompt.length > MAX_PROMPT_CHARS)
  ) {
    return res
      .status(400)
      .json({
        error: `Prompt must be a non-empty string of at most ${MAX_PROMPT_CHARS} characters.`,
      });
  }
  for (const arr of [history, messages]) {
    ...
    if (!Array.isArray(arr) || arr.length > MAX_HISTORY_ITEMS) {
      return res
        .status(400)
        .json({ error: `History must be an array of at most ${MAX_HISTORY_ITEMS} messages.` });
    }
    for (const m of arr) {
      if (m && typeof m.content === 'string' && m.content.length > 8000) {
        return res
          .status(400)
          .json({ error: 'A history message exceeds the maximum length of 8000 characters.' });
      }
    }
  }
  next();
...
<body>
  ...
  if (startedAt !== undefined) {
    ...
    if (Number.isFinite(numStartedAt)) {
      ...
      if (elapsed >= 0 && elapsed < 2500) {
        ...
        return sendError(
          400,
          'Form submitted too quickly. Please review your message and try again.',
        );
      }
    }
  }
  ...
  if (hits.length >= maxRequests) {
    ...
    return sendError(
      429,
      'Too many contact submissions. Please wait 5 minutes before trying again.',
    );
  }
  ...
  try {
    ...
    const contactsList = fs.existsSync(contactsFile)
...
app.post('/api/chat', chatInputGuard, aiRateLimiter, async (req, res) => {
  ...
  try {
    ...
    console.warn(
      'Gemini Chat failed, trying OpenAI fallback...',
  ...
  try {
    ...
    console.warn(
      'OpenAI Chat failed, trying Claude fallback...',
...
5. No HTML tags or scripts inside the SVG.`;
  ...
  try {
    ...
    try {
      ...
      console.warn(
        'Gemini 3.6 Flash fallback to gemini-2.5-flash for icon gen:',
        err.message || err,
      );
      const response = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
...
app.post('/api/chat/stream', chatInputGuard, aiRateLimiter, async (req, res) => {
  ...
  try {
    ...
    console.warn(
      'Gemini Stream failed, trying OpenAI fallback...',
      geminiError.message || geminiError,
    );
  }
  ...
  try {
    ...
    console.warn(
      'OpenAI Stream failed, trying Claude fallback...',
      openaiError.message || openaiError,
    );
  }
  ...
  try {
    ...
    console.warn(
      'Claude Stream failed, using local Mock fallback...',
...
app.post('/api/chat/openai', chatInputGuard, aiRateLimiter, async (req, res) => {
  ...
  if (!apiKey) {
    return res
      ...
      .json({
        error:
          'OpenAI API Key is missing. Provide it on the client or set OPENAI_API_KEY in the server environment.',
      });
  }
  ...
  try {
    ...
    if (!res.headersSent) {
      res
        .status(500)
        .json({ error: err.message || 'Internal server error connecting to OpenAI API' });
    } else {
...
app.post('/api/chat/claude', chatInputGuard, aiRateLimiter, async (req, res) => {
  ...
  if (!apiKey) {
    return res
      ...
      .json({
        error:
          'Claude API Key is missing. Provide it on the client or set ANTHROPIC_API_KEY in the server environment.',
      });
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errText = await response.text();
      res.write(
        `data: ${JSON.stringify({ error: `Anthropic API returned error: ${errText}` })}\n\n`,
      );
      return res.end();
    }
    ...
    if (!res.headersSent) {
      res
        .status(500)
        .json({ error: err.message || 'Internal server error connecting to Anthropic API' });
    } else {
      res.write(`data: ${JSON.stringify({ error: err.message })}\n\n`);
      res.end();
    }
...
app.post('/api/agents/execute', checkAuth, async (req, res) => {
  ...
  if (lowercaseTask.includes('git')) {
    return exec(
      'git status',
      { cwd: path.join('D:', 'DLinacre', 'linacre-site') },
      (error, stdout, stderr) => {
        if (error) {
  ...
  if (lowercaseTask.includes('docker') || lowercaseTask.includes('container')) {
    return exec('docker ps --format "table {{.Names}}\t{{.Status}}"', (error, stdout, stderr) => {
      if (error) {
        return res.json({
          reply: `[Docker Engine Offline]: Docker daemon is not active. Output: ${stderr || error.message}`,
        });
      }
      return res.json({
        reply: `[Docker Live Container Audit]:\n${stdout || 'No running containers found.'}`,
      });
    });
  }
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: `You are an AI system agent named ${agentName || 'Bot'}. Execute this task briefly under 2 paragraphs and explain the steps: "${task}"`,
            },
          ],
        },
      ],
...
app.get('/api/active-logs', checkAuth, async (_req, res) => {
  try {
    ...
    const dirs = items
      ...
      .filter(
        (item): item is any =>
          !!item && item.stat.isDirectory() && item.name !== 'tempmediaStorage',
      )
...
app.all('/api/evbot-proxy/*', async (req, res) => {
  const subPath =
    (req.params as unknown as string[])[0] || req.path.replace(/^\/api\/evbot-proxy\//, '');
...
app.post('/api/auth', (req, res) => {
  const secret =
    process.env.DASHBOARD_SESSION_SECRET || 'default_session_secret_32_bytes_min_12345';
  const passwordHash =
    process.env.DASHBOARD_PASSWORD_HASH ||
    'e716eb6bf2edcc90a2998bf03f1621e0c1858fa503bc39f492f1da71f38a89d5';
  ...
  const suppliedLowerHash = crypto
    .createHash('sha256')
    .update(cleanPassword.toLowerCase())
    .digest('hex');
  const masterChristopherHash = 'e716eb6bf2edcc90a2998bf03f1621e0c1858fa503bc39f492f1da71f38a89d5';
  const masterChristopherLowerHash =
    '5d8bccf165fda6bf1dabc782be82e45e70487aa6b2518704bd269745d583f208';

  const isValid =
    timingSafeEqualStr(suppliedHash, passwordHash) ||
...
app.get('/api/auth', (req, res) => {
  ...
  const valid =
    timingSafeEqualStr(sig, sign(expiresStr, secret)) && Number(expiresStr) > Date.now();
  return res.status(200).json({
    authenticated: valid,
    keysEnabled: valid,
    activeKeys: valid
...
app.post('/api/chat/unified', chatInputGuard, aiRateLimiter, async (req, res) => {
  ...
  if (!catalog) {
    return res
      .status(400)
      .json({
        error: `Unknown provider "${provider}". Valid: ${Object.keys(MODEL_CATALOG).join(', ')}`,
      });
  }
  ...
  if (tier === 'premium' && !isDashAuthed(req)) {
    return res
      ...
      .json({
        error:
          'This model is owner-gated. Unlock with your dashboard password in the config panel.',
      });
  }
  ...
  const sse = (obj: unknown) => {
    if (active) res.write(`data: ${JSON.stringify(obj)}\n\n`);
  };
  const startSSE = () => {
    if (!res.headersSent) {
      res.setHeader('Content-Type', 'text/event-stream');
  ...
  try {
    ...
    if (provider === 'claude') {
      ...
      if (!response.ok) {
        ...
        return res
          .status(502)
          .json({ error: `Anthropic API error ${response.status}: ${errText.slice(0, 400)}` });
      }
      startSSE();
      const decoder = new TextDecoder();
      let buffer = '';
        ...
        while (boundary !== -1) {
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              if (parsed.type === 'content_block_delta' && parsed.delta?.text)
                sse({ text: parsed.delta.text });
            } catch {
              /* partial or ping event */
            }
          }
          boundary = buffer.indexOf('\n');
        }
    ...
    if (!response.ok) {
      ...
      return res
        .status(502)
        .json({ error: `${catalog.label} API error ${response.status}: ${errText.slice(0, 400)}` });
    }
    startSSE();
    const decoder = new TextDecoder();
    let buffer = '';
