(() => {
  ...
  function drawHelix(champ, t) {
    ...
    for (let i = 0; i < rungN; i++) {
      ...
      const pb = project(B.x, B.y, B.z);
      const active =
        genes.length && (Math.floor(t * 2.6) + i) % genes.length === i % Math.max(1, genes.length);
      segs.push({
        a: pa,
        b: pb,
        z: (pa.z + pb.z) * 0.5,
  ...
  function drawOrbs(orgs, t) {
    ...
    for (const item of items) {
      ...
      const r =
        (4.8 + org.fit * 9.5 + (org.elite ? 2.2 : 0) + v.born * 6) *
        p.s *
        17 *
        (0.88 + beat * 0.14);
    ...
    $('hall')
      ...
      .forEach(li => {
        li.addEventListener('click', () => {
          ...
          const found =
            (snap.organisms || []).find(o => o.id === selected) ||
  ...
  $('b-enter').addEventListener('click', () => {
    ...
    idleAt = performance.now();
    try {
      localStorage.setItem('helix-seen', '1');
    } catch (_) {}
  });
  ...
  seedField();
  try {
    if (localStorage.getItem('helix-seen')) $('welcome').hidden = true;
  } catch (_) {}
