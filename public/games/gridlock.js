(() => {
  ...
  const sidesDrawn = (r, c) =>
    boxSides(r, c).reduce((k, s) => k + (game[s.a][s.i] >= 0 ? 1 : 0), 0);
  ...
  function freeEdges() {
    const out = [];
    game.h.forEach((o, i) => {
      if (o < 0) out.push({ a: 'h', i });
    });
    game.v.forEach((o, i) => {
      if (o < 0) out.push({ a: 'v', i });
    });
    return out;
  }
  ...
  function build() {
    ...
    for (let r = 0; r < n; r++)
      for (let c = 0; c < n; c++) {
        ...
        rect.setAttribute('x', p(c));
        rect.setAttribute('y', p(r));
    ...
    for (let r = 0; r <= n; r++)
      for (let c = 0; c < n; c++) addEdge('h', hi(r, c), p(c), p(r), p(c + 1), p(r));
    for (let r = 0; r < n; r++)
      for (let c = 0; c <= n; c++) addEdge('v', vi(r, c), p(c), p(r), p(c), p(r + 1));

    for (let r = 0; r <= n; r++)
      for (let c = 0; c <= n; c++) {
        const d = document.createElementNS(NS, 'circle');
        d.setAttribute('cx', p(c));
        d.setAttribute('cy', p(r));
  ...
  function play(axis, i, byAI) {
    ...
    if (claimed) tone(560 + claimed * 90, 0.09, 'triangle', 0.04);
    else tone(300, 0.05);
  ...
  function chooseAI() {
    ...
    const safe = free.filter(e => touching(e.a, e.i).every(([r, c]) => sidesDrawn(r, c) < 2));
  ...
  function finish() {
    ...
    if (game.vsAI) {
      ...
      dom.sub.textContent = draw
        ...
        : `${Math.max(a, b)} squares to ${Math.min(a, b)}.`;
    }
    store.write(data);
    dom.live.textContent = `${dom.title.textContent}. ${a} to ${b}.`;
    [523, 659, 784].forEach((f, k) => tone(f, 0.15, 'triangle', 0.03, k * 0.07));
  ...
  // ---------- wiring ----------
  el('b-new').addEventListener('click', () => {
    build();
    show(false);
  });
  ...
  dom.btn.addEventListener('click', () => {
    if (dom.btn.dataset.action === 'again') {
      build();
      show(false);
    } else show(false);
  });
