(() => {
  ...
  function hit(i) {
    ...
    if (game.phase === 'wait') {
      // too soon, or fell for the fake
      game.phase = 'between';
      clearTimer();
      game.gen++;
      tone(150, 0.3, 'sawtooth', 0.04);
      buzz([40, 60, 40]);
      if (game.mode === 'duel') {
        const other = 1 - i;
        game.score[other]++;
  ...
  function finish(winner) {
    ...
    if (game.mode === 'duel') {
      ...
      [523, 784].forEach((f, k) => tone(f, 0.18, 'triangle', 0.04, k * 0.1));
    } else {
      ...
      game.avg = avg;
      game.bestOfRun = best;
      [523, 659, 784].forEach((f, k) => tone(f, 0.16, 'triangle', 0.035, k * 0.08));
    }
    store.write(data);
  ...
  function setMode(mode) {
    ...
    bothSides(
      'TAP START',
      '',
      mode === 'duel' ? 'one phone, two thumbs' : 'five signals, best average',
    );
    renderPips();
    show(false);
  }
  ...
  async function copyResult() {
    ...
    setTimeout(() => {
      dom.share.textContent = 'COPY RESULT';
    }, 2000);
  }
