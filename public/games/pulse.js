(() => {
  ...
  function lock(on) {
    dom.pads.classList.toggle('locked', on);
  }
  ...
  function updateStats() {
    const data = store.read();
    if (game.mode === 'duel') {
      dom.kLeft.textContent = 'SIGNAL';
      dom.sLeft.textContent = game.seq.length;
      dom.kRight.textContent = 'TURN';
      dom.sRight.textContent = `P${game.turn + 1}`;
    } else {
      dom.kLeft.textContent = 'SIGNAL';
      dom.sLeft.textContent = game.depth;
      dom.kRight.textContent = 'BEST';
      dom.sRight.textContent = data.best || 0;
    }
  }
