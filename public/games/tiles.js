(() => {
  ...
  const store = {
    read() {
      try {
        return { ...memory, ...JSON.parse(localStorage.getItem(STORE_KEY) || '{}') };
      } catch {
        return memory;
      }
    },
  ...
  function checkState() {
    ...
    setTimeout(() => tone(160, 0.2, 'sawtooth'), 120);
