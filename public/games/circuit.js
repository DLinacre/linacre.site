(() => {
  ...
  const store = {
    read() {
      try {
        return JSON.parse(localStorage.getItem(STORE_KEY)) || memory;
      } catch {
        return memory;
      }
    },
  ...
  function label(c) {
    const row = ((c.i / game.size) | 0) + 1,
      col = (c.i % game.size) + 1;
    const m = maskOf(c);
    const open = DIRS.filter(d => m & d.b)
      .map(d => d.name)
      .join(', ');
  ...
  async function copyResult() {
    ...
    setTimeout(() => {
      el.ovShare.textContent = 'COPY RESULT';
    }, 2200);
  }
  ...
  function win() {
    ...
    if (game.mode === 'daily') {
      ...
      beat =
        !prev || game.moves < prev.moves || (game.moves === prev.moves && game.elapsed < prev.time);
    ...
    if (beat)
      bits.push(game.mode === 'daily' ? 'Best run today.' : `Best ${game.size}×${game.size} yet.`);
  ...
  function load(opts) {
    ...
    el.level.textContent = daily
      ? `${today().slice(5)} · ${size}×${size}`
      : `${opts.level} · ${size}×${size}`;
  ...
  function hint() {
    if (game.solved) return;
    const wrong = game.cells.filter(
      c => c.period > 1 && ((c.spin % c.period) + c.period) % c.period !== 0,
    );
    if (!wrong.length) {
      el.status.textContent = 'EVERY TILE IS ALREADY IN A SOLVED POSITION';
      return;
    }
  ...
  el.daily.addEventListener('click', () => {
    if (game.mode === 'daily')
      load({ mode: 'campaign', level: game.level || store.read().level || 1 });
    else load({ mode: 'daily' });
  });
  document.getElementById('b-new').addEventListener('click', () => {
    if (game.mode === 'daily') {
      el.status.textContent = 'THE DAILY GRID IS FIXED · RESET IT OR SWITCH TO CAMPAIGN';
      return;
    }
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Escape' && open && el.ovBtn.dataset.action === 'close') {
      showOverlay(false);
      return;
    }
  ...
  try {
    ...
    load(
      wantDaily
        ? { mode: 'daily' }
        : { mode: 'campaign', level: Math.max(1, Math.min(999, saved.level || 1)) },
    );
