(() => {
  ...
  function build() {
    cols = LEVELS[mode].cols;
    rows = LEVELS[mode].rows;
    nMines = LEVELS[mode].mines;
    grid = [];
    for (let r = 0; r < rows; r++)
      for (let c = 0; c < cols; c++) grid.push({ mine: false, open: false, flag: false, n: 0 });
    started = false;
    over = false;
    flags = 0;
    opened = 0;
  ...
  function win() {
    ...
    if (isBest) {
      s.best[mode] = t;
      store.write({ ...s, best: s.best });
    }
    tone(660, 0.09);
    setTimeout(() => tone(880, 0.14), 100);
    el.ovTime.textContent = t + 's';
    el.ovBest.textContent = isBest ? t + 's ★' : best + 's';
    showOverlay(
      'FIELD CLEARED',
      (isBest ? 'New best time! ' : '') + 'All ' + nMines + ' mines flagged or avoided.',
      false,
    );
  ...
  build();
  try {
    el.board.children[0] && el.board.children[0].focus({ preventScroll: true });
  } catch {}
})();
