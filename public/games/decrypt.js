(() => {
  ...
  function score(guess, code) {
    let exact = 0;
    const cc = new Array(SYMS).fill(0),
      gc = new Array(SYMS).fill(0);
  ...
  function renderHistory() {
    ...
    game.history.forEach((h, n) => {
      ...
      slots.querySelectorAll('.sym').forEach(b => {
        b.disabled = true;
        b.tabIndex = -1;
      });
    ...
    if (!game.over) {
      ...
      const no = document.createElement('span');
      no.className = 'no';
      no.textContent = game.history.length + 1;
      const slots = document.createElement('div');
      slots.className = 'slots';
      for (let i = 0; i < SLOTS; i++) {
        ...
        b.setAttribute(
          ...
          game.guess[i] == null
            ? `Slot ${i + 1}, empty`
            : `Slot ${i + 1}, ${LETTERS[game.guess[i]]}. Tap to clear.`,
        );
        b.addEventListener('click', () => {
          if (game.guess[i] != null) {
            game.guess[i] = null;
            tone(240, 0.04);
            refresh();
          }
  ...
  function place(s) {
    ...
    if (i === -1) {
      dom.status.textContent = 'ALL FOUR SLOTS ARE FULL · TAP ONE TO TAKE IT BACK';
      return;
    }
    game.guess[i] = s;
    tone(380 + s * 60, 0.05);
  ...
  function finish(won) {
    ...
    dom.reveal.querySelectorAll('.sym').forEach(b => {
      b.disabled = true;
      b.tabIndex = -1;
    });
    ...
    if (won) [523, 659, 784, 1047].forEach((f, k) => tone(f, 0.16, 'triangle', 0.03, k * 0.07));
    else tone(180, 0.3, 'sawtooth', 0.03);
    show(true);
  }
  ...
  function resultText() {
    ...
    const line = game.won
      ? `cracked in ${game.history.length}/${TRIES}`
      : `failed ${TRIES}/${TRIES}`;
  ...
  function load(mode, seed) {
    ...
    game.over = false;
    game.won = false;
    dom.modeTag.textContent =
      mode === 'daily' ? 'DAILY' : mode === 'challenge' ? 'CHALLENGE' : 'FREE PLAY';
    const data = store.read();
    dom.streak.textContent = data.streak || 0;
